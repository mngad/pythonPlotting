\chapter{Principal Component Analysis \& Statistical Shape \& Appearance
...
loading response is shape driven.  The negative standard devitation models in
...
values for PC2), which, as with PC1 causes a reduced reponse to augmentation.

The effects seen in PC3 when loading is varied where the +2 and +3 SD models
show a reverse relationship to the other models (+2 and +3 show a greater
response to posterior loads, others show greater response to anterior loads and
a reduced response to posterior loads), show a split between the larger,
negative SD models and the smaller positive SD model.
...
example the combination of shape changes in PC2 with the density distribution
changes.  Potential risks of this however are that such variation types do not
exist in isolation, at least in this dataset, for example a L1 to L5 shape
variation may always exhibit the density shift.  This may be due to the load
sharing variation caused by the bending of the lumbar region, where the central
lumbar vertebra (L3) may experience more central loading, with the L1 and L5
vertebrae experiencing anterior or posterior loads depending on the degree of
lordosis in the lumbar spine.  Another relationship, that may be specific to
this data set is the shift in the density distribution with vertebral body
volume.  This relationship extends from PC2 to PC3 where the larger vertebra
(likely L5 vertebrae) show an increased posterior density, suggesting that that
posterior experienced larger loads in the body.  Again, like PC2, the smaller
vertebrae in PC3 (likely L1 vertebrae) showed greater anterior density and
likely higher anterior loads in the body.
