...
An interaction property was created with tangential behaviour set to frictionless and normal behavior with properties: \mintinline{python}{pressureOverclosure=HARD, constraintEnforcementMethod=Default} and the final option was experimented with, \mintinline{python}{allowSeperation=ON/OFF}.
The \mintinline{python}{allowSeperation} variable determines whether separation is permitted once contact is established and models ran correctly with it set to its default of \mintinline{python}{ON}.
...
%TODO add table of vertebral volumes.

The effect of this change on the relationship between cement fill and change in augmentation stiffness was identified and is shown in Figure %TODO add image of with without cyl fit data
While there was a change in the relationship, a clear trend between the cement fill percentage and change in stiffness for the majority of the vertebrae is still not evident without including other factors (described in Section ).%TODO percentage fill vs change in stiffness
